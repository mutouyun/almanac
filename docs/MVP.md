...
前面验证了“代码 / 前端 / 部署”链路，但**数据持久化**这层尚未验证。在写任何真实业务前，需先确认数据库的核心假设成立。

### 为何要单独验证

1. **纯 Go SQLite 驱动能否交叉编译 + 在 Windows 跑通**：`modernc.org/sqlite` 号称免 CGO 纯 Go，但需实测在 `GOOS=windows` 交叉编译 + 目标服务器上真跑通。
2. **数据能否扁过部署**：CD 会停服务 → 替换 exe → 重启。数据库文件在 `data\`，部署时**不能被覆盖/清空**，重启后上次写入的数据必须还在。
3. **服务运行账号对 data 目录的写权限**：NSSM 服务以某账号运行，需确认能在 `data\` 建库、写文件。

### 部署依赖：无

SQLite 是**嵌入式数据库**，不是独立服务。驱动直接编译进 `almanac.exe`，数据库仅为磁盘上一个文件（如 `data\almanac.db`）。服务器**零安装**，延续单文件部署哲学。

### 验证方案（最小闭环）

- 接入 `modernc.org/sqlite`，启动时在 `data\` 建库 + 一张最简表（如 `visits(id, time)`）。
- 加端点 `GET /db-check`：每次访问往表里插一条记录，返回当前总行数。

### 验证清单

- [ ] 交叉编译带 SQLite 能过 CI（`GOOS=windows`）。
- [ ] 部署到服务器能跑、能在 `data\` 建库。
- [ ] 多次访问 `/db-check`，行数**递增**（证明能写）。
- [ ] 触发一次 CD 重新部署，再访问，行数**从上次基础继续涨**（证明数据扁过了部署，未被清空）。← 灵魂

> 第 4 项通过 = “自托管 + CI/CD 部署 + 数据持久化”核心架构假设成立，后续真实业务才有地基。

### ⚠️ 部署脚本需适配

当前 CD 只替换 exe，不碰 `data\`——需确认部署脚本不会误删 data 目录。新增的 `.db` 文件已在 `.gitignore` 排除（不入版本）。
