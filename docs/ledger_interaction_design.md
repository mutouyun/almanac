# Almanac Ledger 交互设计文档 v1.0

> 上游依据：`docs/ledger_requirements.md` (v1.5)
...
- **请求**：`POST /api/webhook/entry`
  ...
  - Body (JSON)：
    ```json
    { "date": "2026-07-05 14:30", "type": "瑞幸咖啡", "amount": -19.9 }
    ```
- **响应三态**：
  | 场景 | HTTP | 返回体要点 | 手机端可提示 |
  | :--- | :--- | :--- | :--- |
  | 命中分类 | 200 | 归类结果（叶子分类名 + 完整路径） | “已记入：餐饮/饮品/咖啡” |
  | 落入待分类 | 200 | `status: unclassified`（接口实时计算返回，非存储字段；库中以 `leaf_category_id IS NULL` 隐式表达） | “已记账，待补分类” |
...
- **饼图下钻**：点击一级分类扇区 → 展开其子孙分类占比（逐层下钻，最多 5 层）。金额按递归 CTE 从叶子向上汇总。
...
- **方向继承（视觉约束）**：子节点方向锁定为父节点方向，不可单独修改；跨方向拖拽被禁止。
- **叶子才能配规则**：只有叶子节点右侧才出现“正则 + 优先级”编辑区；非叶子节点无此区。
- **普通 vs 高级模式**：默认普通模式（输入关键词，系统自动包成精确匹配正则）；切换高级模式可直接手编正则表达式。
- **叶子化限制**：一旦节点下挂了子节点，其正则/优先级失效（因非叶子）；删光子节点后恢复为可配规则的叶子。
...
- **试跑**：每行跑一遍路由引擎，展示归类结果。
- **待确认清单**：无法自动对齐的行单独列出，支持批量手动指定叶子分类。
